...
class EfficientDensenetBottleneck(Module):
    ...
    def __init__(self, shared_alloc, num_input_channels, num_output_channels):
        ...
        self.register_buffer("norm_running_mean",
                             torch.zeros(num_input_channels))
        self.register_buffer("norm_running_var",
                             torch.ones(num_input_channels))
        self.conv_weight = Parameter(
            torch.Tensor(num_output_channels, num_input_channels, 1, 1))
    ...
    def _reset_parameters(self):
        ...
        stdv = 1.0 / math.sqrt(self.num_input_channels * 1 * 1)
        self.conv_weight.data.uniform_(-stdv, stdv)

    def forward(self, inputs):
        if isinstance(inputs, Variable):
            inputs = [inputs]
        fn = _EfficientDensenetBottleneckFn(
            self.shared_alloc,
            self.norm_running_mean,
            self.norm_running_var,
            stride=1,
            padding=0,
            dilation=1,
            groups=1,
...
class _DenseLayer(Module):
    def __init__(self, shared_alloc, num_input_features, growth_rate, bn_size,
                 drop_rate):
        ...
        if bn_size > 0:
            self.efficient = EfficientDensenetBottleneck(
            ...
            self.efficient = EfficientDensenetBottleneck(
                shared_alloc, num_input_features, growth_rate)
...
class _DenseBlock(Module):
    def __init__(
        ...
        self.final_num_features = num_input_features + (growth_rate *
                                                        num_layers)
...
class DenseNetEfficientMulti(Module):
    ...
    def __init__(
        ...
        assert (0 < compression <=
                1), "compression of densenet should be between "
...
def DenseNet190():
    return DenseNetEfficientMulti(growth_rate=40,
...
class _EfficientCat(object):
    ...
    def forward(self, *inputs):
        ...
        res = type(inputs[0])(
            self.storage.change_device(cur_device_id)).resize_(size)
...
class _EfficientReLU(object):
    ...
    def forward(self, input):
        ...
        output = input
        backend.Threshold_updateOutput(backend.library_state, input, output, 0,
                                       0, True)
        return output
...
class _EfficientConv2d(object):
    ...
    def backward(self, weight, bias, input, grad_output):
        ...
        torch._C._cudnn_convolution_backward_data(grad_output, grad_input,
                                                  weight, self._cudnn_info,
                                                  cudnn.benchmark)

        grad_weight = weight.new().resize_as_(weight)
        torch._C._cudnn_convolution_backward_filter(grad_output, input,
                                                    grad_weight,
                                                    self._cudnn_info,
                                                    cudnn.benchmark)
        ...
        if bias is not None:
            ...
            torch._C._cudnn_convolution_backward_bias(grad_output, grad_bias,
                                                      self._cudnn_info)
        else:
            grad_bias = None
...
class _EfficientDensenetBottleneckFn(Function):
    ...
    def __init__(
        ...
        self.efficient_batch_norm = _EfficientBatchNorm(
            shared_alloc, running_mean, running_var, training, momentum, eps)
        self.efficient_relu = _EfficientReLU()

        self.efficient_conv = _EfficientConv2d(stride, padding, dilation,
                                               groups)

        # Buffers to store old versions of bn statistics
        self.prev_running_mean = self.efficient_batch_norm.running_mean.new()
        self.prev_running_mean.resize_as_(
            self.efficient_batch_norm.running_mean)
        ...
        self.curr_running_mean.resize_as_(
            self.efficient_batch_norm.running_mean)
    ...
    def forward(self, bn_weight, bn_bias, conv_weight, *inputs):
        ...
        bn_output = self.efficient_batch_norm.forward(bn_weight, bn_bias,
                                                      bn_input)
        relu_output = self.efficient_relu.forward(bn_output)
        bias = None
        conv_output = self.efficient_conv.forward(conv_weight, None,
                                                  relu_output)
    ...
    def backward(self, grad_output):
        ...
        bn_output = self.efficient_batch_norm.forward(self.bn_weight,
                                                      self.bn_bias, cat_output)
        relu_output = self.efficient_relu.forward(bn_output)
        ...
        # ReLU backward
        relu_grad_output = self.efficient_relu.backward(
            bn_output, conv_grad_output)

        # BN backward
        cat_output = self.efficient_cat.forward(
            *self.inputs
        )  # recompute cat_output because bn_output override the storage (L481)
        ...
        return tuple([bn_weight_grad, bn_bias_grad, conv_weight_grad] +
                     list(grad_inputs))
